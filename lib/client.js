window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TokenStackCard() {
			...
			var truncated = f.byTool ? Object.keys(f.byTool).length : 0;
			var sessions = stats && stats.bySession ? Object.keys(stats.bySession).length : 0;
			var since = stats && stats.since ? String(stats.since).slice(5, 16).replace("T", " ") : "—";
			var short = function (v) { return v == null ? "—" : String(v); };
			return h("div", {
				style: {
					position: "fixed", top: "10px", right: "10px", zIndex: 60, width: "232px",
				...
				open ? h("div", { style: { display: "flex", flexDirection: "column", gap: "1px" } },
					row("filter saved tok", short(f.tokensSaved)),
					row("filter calls", short(f.calls)),
					h("div", { style: { height: "4px" } }),
					row("mem total", short(m.total)),
					row("mem curated", short(m.curated)),
					row("mem +new", short(m.entriesAdded)),
					row("injected tok", short(m.tokensInjected)),
					row("sessions", String(sessions)),
					row("tools truncated", String(truncated)),
					h("div", { style: { marginTop: "5px", fontSize: "9.5px", opacity: 0.45, lineHeight: 1.45 } },
						"since " + since + " · filter counts >" + short(f.truncateOverChars) + " chars"),
					h("div", { style: { fontSize: "9.5px", opacity: 0.45, lineHeight: 1.45 } },
						"saved = precise · mem total/curated = now · +new = since · terse n/a (A/B)"),
