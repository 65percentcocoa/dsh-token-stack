...
function apply(ctx, config) {
	...
	async function loadAll() {
		...
		if (prior && typeof prior === 'object') {
			stats = Object.assign(stats, prior);
			// Defensive: a stale/partial file must never leave a nested shape undefined.
			stats.filter = Object.assign({ calls: 0, tokensSaved: 0, byTool: {} }, stats.filter || {});
			stats.filter.byTool = stats.filter.byTool || {};
			stats.memory = Object.assign({ sessionsInjected: 0, tokensInjected: 0, entriesAdded: 0, dedupHits: 0, recalls: 0, byTool: {} }, stats.memory || {});
			stats.memory.byTool = stats.memory.byTool || {};
	...
	function statsView() {
		// Current composition of the memory store (a snapshot, not a counter).
		const byKind = {};
		for (const it of memoryCache) {
			const k = it.kind || 'tool';
			byKind[k] = (byKind[k] || 0) + 1;
		}
		const curated = (byKind.fact || 0) + (byKind.preference || 0) + (byKind.constraint || 0);
		return {
			...
			filter: {
				...
				truncateOverChars: config.textMaxChars,
				note: 'tokensSaved = precise estimate of tool-output tokens removed before the model saw them; calls counts only outputs longer than truncateOverChars',
				byTool: stats.filter.byTool,
			},
			memory: Object.assign({}, stats.memory, {
				total: memoryCache.length,
				curated: curated,
				byKind: byKind,
				note: 'total/curated describe the CURRENT store; entriesAdded/dedupHits/recalls are counters since `since`; tokensInjected is a COST, not a saving',
			}),
