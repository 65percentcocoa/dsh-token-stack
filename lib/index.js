...
const inject = ['systemPrompt', 'fs', 'settings', 'tools', 'tokenMeter'];
...
function apply(ctx, config) {
	...
	let stats = {
		since: Date.now(),
		filter: { calls: 0, tokensSaved: 0 },
		memory: { sessionsInjected: 0, tokensInjected: 0, entriesAdded: 0, dedupHits: 0, recalls: 0 },
	...
	function persistStats() { if (statsTarget) writeJson(statsTarget, stats); }
	...
	function recordMemory(exec, result) {
		...
		if (isDuplicate(tool, compact)) { stats.memory.dedupHits++; return; }
	...
	const statsTool = {
		...
		execute: () => JSON.stringify({
			...
			filter: {
				...
				note: 'tokensSaved = heuristic tokens of tool output removed by truncation (precise, not an A/B claim)',
			},
			memory: stats.memory,
			terse: stats.terse,
		}, null, 2),
	...
	ctx.on('tools/post-execute', async (exec, result, next) => {
		...
		stats.filter.calls++;
		stats.filter.tokensSaved += Math.max(0, estimate(original) - estimate(trimmed));
